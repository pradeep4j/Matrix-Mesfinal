...
export const login = async(req, res,next) => {
    try{
         ...
         otherDetails.tokenexp = 20*24*60*60*1000;
         res.cookie('access_token',token,{maxAge : (20*24*60*60*1000) /* cookie will expires in 20 days*/, httpOnly:true}).status(201).json({...otherDetails});
...
export const createLiseReg = async (request, response, next) => {
    try {
        ...
        const regsandrate =  await Lisereg.find();
        const regsn = regsandrate[0].regNo;
        const rates = regsandrate[0].rate;
        if (regsn && rates && documents && docReqDate && docReqFollow && docReviewDate && docStatus && docRemark) {
            const liseReg = {
                regsn, rates, documents: docImageUrl, docReqDate, docReqFollow, docReviewDate, docStatus, docRemark
            }
            console.log(liseReg);
            // const newLiseReg = await Lisereg.(liseReg)
            // await newLiseReg.save()
            response.status(201).json(newLiseReg)
        }
